...
export const Layout: React.FC<{ children: React.ReactNode; onAddClick: () => void }> = ({ children, onAddClick }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen w-full relative selection:bg-indigo-100">
      <main className={`flex-grow w-full ${isChatPage ? '' : 'p-5 pb-40 safe-top'}`}>
        {children}
      </main>

      {!isChatPage && (
        <div className="fixed bottom-6 left-0 right-0 z-50 px-4">
          <nav className="glass-nav rounded-[2.5rem] px-2 py-2 flex justify-between items-center shadow-[0_20px_60px_-15px_rgba(0,0,0,0.2)] ring-1 ring-black/5 max-w-lg mx-auto">
